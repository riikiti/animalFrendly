...
**Аутентификация в Feature-тестах:** для тестов, проверяющих защищённые эндпоинты как
таковые (не сам механизм логина/логаута), — `Laravel\Sanctum\Sanctum::actingAs($user)`, не
ручное создание токена и заголовок `Authorization: Bearer`. Причина: `RequestGuard`
кэширует резолвнутого пользователя на время жизни guard'а, а guard переживает несколько
симулированных запросов внутри одного теста — при смене Bearer-токена на другого
пользователя в середине теста запрос по-прежнему аутентифицируется как первый
пользователь, тест ложно проходит. `Sanctum::actingAs()` эту ловушку обходит и корректно
переключает пользователя при повторном вызове. Ручные токены — только в тестах самого
модуля `Identity` (регистрация/логин/логаут), где токен и есть предмет проверки.
