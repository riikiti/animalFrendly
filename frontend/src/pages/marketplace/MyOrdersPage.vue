...
<template>
  ...
  >
    <div class="flex items-center justify-between px-2">
      <span class="font-display text-xl font-bold text-ink">Мои заказы</span>
    </div>

    <div class="flex gap-2 px-2">
      <button
        class="rounded-full px-3.5 py-1.5 text-sm font-semibold"
        :class="role === 'buyer' ? 'bg-accent text-accent-ink' : 'bg-surface-soft text-ink-soft'"
        @click="role = 'buyer'"
      >
        Покупки
      </button>
      <button
        class="rounded-full px-3.5 py-1.5 text-sm font-semibold"
        :class="role === 'seller' ? 'bg-accent text-accent-ink' : 'bg-surface-soft text-ink-soft'"
        @click="role = 'seller'"
      >
        Продажи
      </button>
    ...
    <div v-if="!isLoading" class="flex flex-1 flex-col gap-3 pb-4">
      ...
      <button
        ...
        :key="order.id"
        class="flex flex-col gap-2 rounded-card border border-hairline bg-surface p-3 text-left"
